...
//     uint8_t vy = (instruction & 0x00F0u) >> 4u;
...
//         break;
//       case 0xA000:
//         OP_Annn(nnn); // set I = nnn
//         break;
//       case 0xB000:
//         OP_Bnnn(nnn);       // jump to address nnn + V0
//         break;
//       case 0xC000:
//         OP_Cxkk(vx, kk); // set Vx = random byte AND kk
//         break;
//       case 0xD000:
//         OP_Dxyn(vx, vy, n); // draw sprite
//         break;
//       case 0xE000: {
//         switch (instruction & 0x00FF) {
//           case 0x009E:
//             OP_Ex9E(vx);       // skip next instruction if key with the value of Vx is pressed
//             break;
//           case 0x00A1:
//             OP_ExA1(vx);       // skip next instruction if key with the value of Vx is not pressed
//             break;
//           default:
//             return ERROR_INVALID_OPCODE;
//         }
...
//           break;
//         case 0x000A:
//           OP_Fx0A(vx);       // wait for a key press, store the value of the key in Vx
//           break;
//         case 0x0015:
...
//           break;
//         case 0x0029:
//           OP_Fx29(vx);       // set I = location of sprite for digit Vx
//           break;
//         case 0x0033:
...
void vm::Cycle() {
  instruction = (uint16_t(memory[pc]) << 8) | uint16_t(memory[pc + 1]);
  vx = (instruction & 0x0F00u) >> 8u;
  vy = (instruction & 0x00F0u) >> 4u;
  kk = instruction & 0x00FF;
  n = instruction & 0x000F;
  nnn = instruction & 0x0FFFu;
  pc += 2;
  ((*this).*(table[(instruction & 0xf000u) >> 12u]))();
  if (delay_timer > 0)
    --delay_timer;
  if (sound_timer > 0)
    --sound_timer;
